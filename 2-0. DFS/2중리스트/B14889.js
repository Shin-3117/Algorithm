const sol = (input) => {
    const N = +input[0]; // 첫 번째 줄
    const halfN = N / 2;
    //input의 2,3,4~ 줄 '0 1 2 3'을 [0,1,2,3]으로 변경
    const stats = input.slice(1).map((str) => str.split(" ").map(Number));
    console.log(stats)
    const check = new Array(N).fill(0); // 파이썬의 check = [0]*N
    let min = Number.MAX_SAFE_INTEGER; // 파이썬의 float('inf')와 같음
    function dfs(L, K) {
